...
- [x] **步骤级即时验证(B-软主体)**(2026-06-15,内网"按步预期"用例驱动)：`SpecStep.expect` 在该步 mark_step_done 落定时于**当前子页面**即时验证(`agent.run::verify_step` 经 `react_loop.on_step_done` 接入),用与终态同款 `AssertionEngine`(**含自愈** → 失败先重定位复验,软、可恢复,符合铁律2(a)),救不回才算该步 FAIL 并**计入裁决**;用例级 `assertions` 仍终态验,两者合并。翻译 prompt 引导预期按步分发到 `expect`。结果带 `phase=step/final`+`step_no` 标注落库 + 前端「步骤N」标签。单测 +3(react_loop budget/on_step_done、agent 步骤级)。**遗留**:① 真实内网用例 live 验(saucedemo 冒烟无回归,但其预期未按步对齐故走终态路径);② 仍受「断言目标定位器对齐」影响(定位不稳的步会自愈兜底,见下条);③ 纯"过早 mark_done 挡"已有 `_guard_premature_mark` 覆盖,这里不重复。
