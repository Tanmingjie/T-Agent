...
- [ ] **运行时锚点自动捕获(URL/数据)** — 翻译期只产意图不接地(FG01),运行时可自动捕获
  ...
  - [x] **登录/导航跳转锚点(2026-06-24 已落地)** — `_check_llm_judge` 加 `prev_url` 参数,
    `agent.on_phase_end` 维护「阶段起始 URL」基线(初始=base_url,每阶段裁完更新为当时 URL),
    起始≠当前时把「页面已从 X 跳转到 Y(确定性事实)」喂裁判;`_JUDGE_SYSTEM` 加一条:**导航/
    登录达成类**期望,URL 已离开起始页(尤其登录页)+ 当前非登录表单页即为有效证据判 PASS,
    无需纠结目标路径"典型"与否、无需逐字找导航菜单文案;**内容/数值类**期望仍须快照证据(不
    因跳转就放绿)。动机案例见下。〔翻译侧引导(写稳态特征/不假设典型路径)`3d79628` 实测不够,
    病在裁判层对陌生页面偏-FAIL → 上此确定性锚点。〕
  - **动机案例(2026-06-24)**:expected「登录成功，进入系统主页面」被裁判判 FAIL,理由
    「当前 URL 为 /about 而非典型主页路径，且快照中无明确系统主功能导航菜单文本证据」。登录其实
    成功了(人眼确认进了主页)——`/about` 是该内网系统的真实落地页,裁判拿"典型主页路径"公网
    先验对赌、又认不出陌生内网导航 → 偏-FAIL 误伤。根治信号不是文案,是**确定性事实**:登录后
    **URL 从登录页跳走**。已落地"登录/导航跳转锚点"(见上)。
  - **未做(余项)**:① **登录表单消失**的确定性检测(当前只用 URL 跳转;URL 跳转+密码框消失
    双信号更强,但需探起始页是否有 password 输入框);② **关键数据锚点**自动回填(到达页关键
    数值/标识),供内容/数值类期望减少对 LLM 语义核验的依赖。
