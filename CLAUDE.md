...
- **翻译阶段提速 + 铁律2 重订 + 断言去冗余(2026-06-10,从内网「翻译总超时」问题展开)** —
  ...
  - **未来项(已记 TODO)**:**B-软(步骤级软校验)** — `expect` 在步骤边界软校验,不过→自愈/重试/绕行(**不**判失败);挡「过早 mark_done」+ 补瞬态期望覆盖。**前置依赖「断言目标定位器对齐」**(定位不稳时步骤级硬信号误判空转);贯彻铁律2(a)软、可恢复。
