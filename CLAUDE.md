...
- **下一步候选(2026-06-07 收口时的待办):**
  - **Fix 3 = 实装新裁决架构** — ✅ **已做**(含收尾加固,见上「实施进度」Fix 3 两条目)。**剩余可选**:⑤ 用 `eval_fg/` 扩样(换 Cisco/ThingsBoard + 第二模型)把 false-green/false-fail 率做到可信区间(尤其量化新加的「证据核验」对弱模型的误伤率)。
...
- **让 LLM 裁判判 PASS 时逐字引证、平台再确定性核验**(Fix 3 收尾):光靠 prompt「偏-FAIL + 引证证据」治不住弱模型脑补——它会一边判 PASS 一边编造"用户名框=standard_user"(当前页根本没这字段)。robust 解=要求裁判把依据**逐字摘录**进 `evidence` 字段,代码**确定性核验**该串真在当前页快照/URL 里(空白归一子串匹配),不在 → fail-closed 推翻 FAIL。把"信不信模型"变成"能不能在页面里找到它说的话",可审计、跨模型稳。核验只在确有页面文本时启用(无快照单测不误伤)。
