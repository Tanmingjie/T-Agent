...
- **下一步候选(2026-06-07 收口时的待办):**
  ...
  - **断言目标定位器对齐(遗留)** — codegen 的 action 定位器已对齐(执行捕获),但**断言目标**(如购物车角标)不经"执行定位器"产出,无 vocab/selector 时仍文本兜底。需在 vocab/selector 层对齐(断言走 probe,naming 随 LLM 漂移、子串匹配也常错过)。
