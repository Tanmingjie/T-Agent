...
async def test_execute_run_preserves_selected_scope_order_and_parallelism(tmp_path, monkeypatch):
    ...
    await repo.bulk_insert(
        [
            TestCase(id="t1", name="C1", steps=["a"], base_url="https://x.com", suite_id="sx"),
            TestCase(id="t2", name="C2", steps=["b"], base_url="https://x.com", suite_id="sx"),
            TestCase(id="t3", name="C3", steps=["c"], base_url="https://x.com", suite_id="sx"),
        ]
    )
    await set_suite_settings(store, "sx", "trust", parallelism=2)
    await repo.create_run("selected-run", "sx", 2, None, None)
