...
- **ThingsBoard 工业 SPA 替身战役(2026-06-28→30)** ★ — 接 SVG/HMI 批次的「下周接力点③」,
  对 `thingsboard.cloud`(Angular Material SPA + mat-table + canvas/SVG widget + onboarding 浮层,
  最接近内网工艺模拟器的**脏工业 web**)写真用例(TB01–TB05,生成器 `examples/make_thingsboard_xlsx.py`
  `77f875a`,凭据走 `TB_EMAIL/TB_PASSWORD` env、含明文凭据的 xlsx 不入库)端到端跑,挖出并修一串
  dirty-SPA 真根因。**约束:内网数据全程不外泄,只用公开替身复现失败类**(见 [[public-analogue-debugging]])。
  **认知校正延续:用户内网模型 deepseek-v4-pro / qwen3.5-397B 是强模型,失败=页面可观测性/执行健壮性,非模型能力。**
  - **工具失败判定改信 MCP 结构化 `isError`(`44cc9ba`)** ★ — `_is_tool_failure` 旧实现拿**整段观察
    (含 playwright-mcp 随成功结果返回的整页 a11y 快照)**扫 `not found`/`error`/`timeout` 裸子串 →
    只要**页面内容**出现这些词(工业页报警/状态文本高发),每帧观察被误判工具失败 → 跑 healer 回灌
    "重定位建议" → 模型以为 `browser_wait_for` 一直失败、反复重试(死循环)。根治两层:①主信号改用
    MCP `CallToolResult.isError`(透 `ToolOutcome.is_error`→`_outcome_failed`,权威、不受页面内容影响);
    ②字符串 marker 降为兜底,先剥 ```yaml 围栏(`_strip_snapshot`)只扫错误信封。见 [[snapshot-truncation]]
    工程经验条「失败判定要信工具的结构化失败标志」。
  - **Context Compact 加硬字符上限(`984f300`)** ★ — `truncate_snapshot` 旧实现**只按行数**截断,
    被**单行 megablob**(压缩 JS/巨型 JSON 整坨无换行)绕过 → 单条观察几十万 token → 冲破 1M 上下文 →
    `ContextWindowExceededError` 整条 run 不可恢复崩溃(实测 1.25M tokens)。根治:`_cap_long_lines`
    就地砍超长单行 + `_cap_total` 末位总字符上限(`OBS_HARD_CHAR_CAP` 默认 16000)。教训:任何"截断/
    压缩"逻辑都要有一道**不依赖输入结构(行数/字段数)的硬字节上限兜底**。
  - **驱动侧快照截断默认放大(`27f62eb`+`654e67a`)** — 企业级密集 SPA(设备详情/mat-table)长页被
    截 → 导航/硬交互元素看不到。`SNAPSHOT_MAX_LINES` 默认 40→80→**150**(截断优先保留可交互元素行)。
  - **STEP_FAIL_BUDGET 默认调高(`ce77c7c`+`654e67a`)** — 内网脏 live SPA stale ref 致 click-by-ref
    ~5s 超时,模型靠 URL-navigate 合法恢复但被预算掐断。默认 5→8→**10** 给恢复留头寸。
  - **裁判侧快照上限放大(`af6c0d9`)** — `JUDGE_SNAPSHOT_LIMIT` 9000→**20000**,治跨语言(中文 expected
    ↔ 英文页面)锚点窗口截断漏看证据。与驱动侧两条独立路径。
  - **内置基线 Skill 加「先关挡住点击的浮层」(`1f7d124`)** — welcome/onboarding cdk-overlay-backdrop
    拦截点击 → 加 preload skill 引导模型先点关闭(×/关闭/Close/Skip/Got it/Escape)再重新快照。
  - **入库** `77f875a`:TB01–TB05 生成器(脱敏,凭据走 env)。
  - **验收**:deepseek-v4-pro + 新默认下 **TB01/TB02/TB04(综合复杂流程,13 步:登录→设备表→HVAC 行钻取
    详情+遥测→仪表盘列表→等待 3 分钟→存活)/ TB05(48 步硬交互探针)全 live PASS**。3 分钟长等待 + 存活
    验证了 isError/megablob/context 三修在真实脏 SPA 长 idle 下成立。**canvas widget 确认为 a11y 盲区**
    → expected 只写 a11y 可观察证据(表行/标题/URL/遥测键值),不碰图表内部值。
  - **接力点**:① **用户内网拉最新代码验证**这批 dirty-SPA 默认(快照 150/预算 10/isError/megablob 上限)
    在真实工艺页效果,按非敏感信号反馈继续挖第三个根因。② **canvas/纯 SVG widget 裁决**需 `JUDGE_VISUAL=1`
    多模态——已定位未做的下一个点,待用户拍板。
