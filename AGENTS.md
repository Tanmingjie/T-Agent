...
```
业务用例(Excel) → TestSpec(阶段化执行规格+阶段预期) → Midscene 视觉执行(aiAct/aiAssert)
→ 结构化执行记录 + Midscene 报告/截图 → 前端可视化结果
```
...
→ VisualExecutor → scripts/midscene_runner.js → @midscene/web PlaywrightAgent
```

保留平台能力:Excel/TestCase/Suite/Run、TestSpec 翻译、项目用例规范、执行前勾选 Skill、SSE/run_event、
ExecutionRecord、截图/report artifacts。降级/移除:playwright-mcp 执行、ReAct tool-call 权限审批、
Custom Tool、词汇表 DOM 定位、旧 healing、旧 codegen 主链路。

## 历史状态:内网真实环境加固(2026-06-24 起)
...
单条用例的执行由 `api/run_executor.py::execute_run()` 总装,当前固定创建
`harness/midscene_agent.py::MidsceneCaseAgent`。核心模块：
...
- `harness/midscene_agent.py` — Midscene 执行适配器。复用 TestSpec 翻译/ExecutionRecord/run_event 契约,把每条用例交给 `VisualExecutor`;将 phase result 归一为阶段断言,metrics 标记 `execution_kernel=midscene`。
...
- **较大改动(碰执行链 / 断言 / 翻译 / API 执行路径)交付前,除单测外优先跑一轮 Midscene live 冒烟**(真内网视觉模型 + 真浏览器,验端到端不退化,弥补「单测绿≠接通」)。当前 Midscene live 依赖 `MIDSCENE_ENABLED=1` 与 `MIDSCENE_MODEL_*` 配置;若本机/当前会话没有可用视觉模型,必须在交付说明里明确“未跑真实 Midscene live”。
...
```
T-agent/
├── harness/          # 执行与记录核心(Midscene adapter / VisualExecutor / LLM / 录制…)
│   ├── hooks.py        #   生命周期 Hook(通用扩展点;参考 Codex hooks,平台只提供机制)
├── mcp_client/       # 旧 ReAct/playwright-mcp 链路遗留,当前主执行路径不再使用
