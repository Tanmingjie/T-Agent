...
- 有序 ``phases``:每个阶段 = 一组步骤(自然语言,数据内联,**驱动**)+ 一条组级预期
  ``expected``(只给阶段边界 Validator 作偏-FAIL 裁判依据,要求引证页面证据;**不进驱动**)。
...
- 每条步骤聚焦一个连续子任务,不要把跨页面、跨弹窗的多个独立操作压成一条超长步骤。运行时会逐条
  执行这些步骤,完成当前步骤后再进入下一条。
...
- 每个阶段给**一条** expected:该阶段子目标达成后、**在当时所处页面上可观察到**的状态。系统在该
  阶段结束、页面停在那一刻时,用**无障碍快照 + URL**由偏-FAIL 裁判核验它;它**只是验证依据,不会**
  拿来驱动 agent。
...
① ⚠️【只用可核验的判据】只能用**有文字的元素、URL 片段、页面可见文案**作判据。**禁止**写
  ...
  无障碍快照,核验不到图标就会判失败。⚠️ 说"出现 XX"时**直接写页面上的文字本身**(如「页面出现
  文案 Products」),**别用"标题"二字**——"标题"会被理解成浏览器文档标题(可能与页面内文案不同)而误判。
  例:✓"URL 含 inventory.html,页面出现文案 Products" ✗"出现 Products 标题"(歧义) ✗"右上角出现
  购物车图标"(纯图标无文字)。(带数字/文字的角标算有文字、可核验,如"角标显示数字 1"可用。)
...
【执行稳定性改写规则】
- 对 checkbox / switch / radio / 开关 / 勾选 这类**重复点击会反转状态**的控件,步骤必须写成
  **目标状态**,不要写成单纯"点击"。例:✓"确保告警开关处于开启状态;如果已经开启,不要再次点击"
  ✓"确保自动刷新复选框处于未勾选状态;如果已经未勾选,不要再次点击"。这样运行时会按目标状态收敛,
  避免控件已开启时再次点击导致关闭。
- 对"等待 3 分钟 / 观察 30 秒 / 等待 180s 生成数据"这类**带明确时长**的步骤,必须把等待/观察
  作为独立 steps 保留下来,并保留原始时长。不要把它改写成"确认数据已生成",也不要和前后动作揉成
  一句"点击并等待后查看"。运行时会把明确时长的等待/观察转成确定性 sleep,不交给 aiAct 猜。
- 对"等到数值低于 30 / 直到状态变为已完成 / 等报警出现后再点击确认"这类**条件等待/轮询观察**,
  必须保留条件和后续动作。不要改写成固定等待时长;运行时会使用 Midscene aiWaitFor 观察页面直到
  条件成立,再继续执行后续动作。
...
【业务知识/操作指南(若用户提供)】
- 用户可能在用例之外提供一段**本系统的业务知识/操作指南**(流程怎么走、术语对应、成功态长什么样)。
- 用它**理解流程、对齐术语、判断成功标准**:把用例里隐含但本系统必需的步骤**补全**(如"提交前需先
  选审批人");把业务黑话翻成执行器认得的措辞;据它把 expected 写成**本系统真实可观察**的结果。
- ⚠️ 但**两条铁律不变**:① 仍**不写 selector、不锁动作类型、不猜元素 ID**——知识只在自然语言层提升
  步骤/意图质量,接地仍交运行时;② expected 仍**只能写页面真实可观察、由本阶段步骤直接产生**的状态,
  **不要**把指南里描述的理想态/后续态当成"页面一定会出现"而写进 expected(否则制造抓不到的硬门槛→误判失败)。
...
def build_spec_messages(case: TestCase, *, knowledge: str = "") -> list[dict]:
    """组装给 LLM 的消息(纯函数,便于单测)。

    ``knowledge``:可选的本系统业务知识/操作指南(自然语言)。给定时作为**理解用背景**置于
    用例之前,引导翻译补全流程/对齐术语/写对 expected——但仍受系统 prompt 两条护栏约束
    (不接地、不脑补 expected)。
    """
...
class SpecGenerator:
    ...
    async def generate(self, case: TestCase, *, knowledge: str = "", on_delta=None) -> TestSpec:
        """生成阶段化 TestSpec。LLM 或解析失败时按配置降级或抛出。

        ``knowledge``:可选的本系统业务知识/操作指南,注入翻译 prompt 助补全流程/对齐术语/写对
        expected(受系统 prompt 两条护栏约束,不接地、不脑补)。
        ``on_delta``:给定则走流式(逐 token 回调),让慢模型长生成不被网关空闲超时切断。
        """
        messages = build_spec_messages(case, knowledge=knowledge)
        try:
            resp = await (
