...
- expected 要可观察、具体到该阶段所在页面,可含多个事实。例如:
  · 「完成登录」阶段 → "登录成功，进入商品列表页（URL 含 inventory.html，出现商品列表）"
  · 「把商品加入购物车」阶段 → "购物车角标显示 1，该商品按钮变为 Remove"
  (注意上面两条各自只描述**它那个阶段**的状态:登录阶段绝不提购物车,加购阶段才提角标。)
- ⚠️【作用域锁定:expected 只能断言本阶段自己步骤产生的状态】一条 expected 只能写**本阶段
  的步骤**直接达成、在本阶段所处页面**当时就能看到**的东西;**绝不能引用后续阶段步骤才会
  出现的元素/状态**。反例(错):登录阶段(步骤只到点登录)的 expected 写"页面出现购物车图标/
  角标"——购物车图标要等"加入购物车"阶段才会有,写进登录阶段 = 越界 → 会把一次正常登录判失败。
  自检:写完每条 expected,问自己"这东西在**本阶段最后一步刚做完**的那一刻页面上真的有吗?"
- ⚠️【缺源头预期就保守,别脑补业务断言】如果用例的「预期结果」**没有**覆盖到某个(中间)阶段,
  **不要凭业务想象给它编一条丰富断言**——只写**该阶段最后一步本身达成的最小可观察结果**
  (如"已进入采购单页面""表单已提交、出现下一步""列表已加载出数据行")。宁可朴素、确定能观察到,
  也不要为了"显得业务"而写出页面上根本不一定有的东西(脑补的断言会无端把正常流程判失败)。
