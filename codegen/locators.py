...
_NAME_OPT_RE = re.compile(r"""name\s*:\s*(['"])(.*?)\1""")


def locator_from_executed(expr: str, target: str = "") -> Locator | None:
    """把**实际执行的 Playwright 定位表达式**解析成 Locator(ground truth,见 react_loop)。

    支持 playwright-mcp 常见输出:
    - ``page.getByRole('button', { name: 'Login' })`` → ROLE
    - ``page.getByTestId('x')`` → TEST_ID;``getByLabel`` → LABEL;``getByPlaceholder`` → PLACEHOLDER;
      ``getByText`` / ``getByTitle`` / ``getByAltText`` → TEXT
    - ``page.locator('[data-test="username"]')`` → CSS(保留原选择器,真跑通过、必然可用)

    这些来自实际执行、必然唯一可用,故 ``fallback=False``(不打"待核对"注释)。
    """
    if not expr:
        return None
    e = expr.strip()
    if "getByRole(" in e:
        m = _QUOTED_RE.search(e)
        role = m.group(2) if m else ""
        nm = _NAME_OPT_RE.search(e)
        name = nm.group(2) if nm else ""
        if role:
            return Locator(LocatorStrategy.ROLE, role=role, name=name, target=target)
    _SIMPLE = {
        "getByTestId(": LocatorStrategy.TEST_ID,
        "getByLabel(": LocatorStrategy.LABEL,
        "getByPlaceholder(": LocatorStrategy.PLACEHOLDER,
        "getByText(": LocatorStrategy.TEXT,
        "getByTitle(": LocatorStrategy.TEXT,
        "getByAltText(": LocatorStrategy.TEXT,
    }
    for marker, strat in _SIMPLE.items():
        if marker in e:
            m = _QUOTED_RE.search(e)
            if m and m.group(2):
                return Locator(strat, name=m.group(2), target=target)
    if "locator(" in e:
        m = _QUOTED_RE.search(e)
        if m and m.group(2):
            return Locator(LocatorStrategy.CSS, value=m.group(2), target=target)
    return None


def locators_from_steps(steps) -> dict[str, Locator]:
    """执行期捕获的 ActionStep → ``{业务 target: Locator}``(框架无关)。

    优先级:**实际执行的定位表达式**(``element_selector``,ground truth,真跑通过、必然唯一可用)
    > 快照回查的 ``role+name``(语义稳健但可能有歧义,如多个"Add to cart")。二者都给未录入
    词汇表的目标产出稳健定位,整体优先级 执行捕获 > 词汇表 > 文本兜底。同一 target 取首个成功。
    """
    ...
    for s in steps:
        ...
        if not target or target in out:
            continue
        # 1) 实际执行的定位器(ground truth)
        loc = locator_from_executed(getattr(s, "element_selector", "") or "", target)
        # 2) 退回快照 role+name(仅 role 与 name 都齐备时)
        if loc is None:
            role = (getattr(s, "element_role", "") or "").strip()
            name = (getattr(s, "element_name", "") or "").strip()
            if role and name:
                loc = Locator(LocatorStrategy.ROLE, role=role, name=name, target=target)
        if loc is not None:
            out[target] = loc
    return out
